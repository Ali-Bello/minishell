...
/*   Updated: 2024/11/26 21:49:02 by aderraj          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	arrange_nodes(t_list *list[3], t_redir **redirections, t_mini *mini)
{
	if (list[0] && list[0]->type == WORD)
	{
		list[0] = get_redirections(list[1], list[0], redirections, mini);
		merge_words(list[0], *redirections, mini);
		redirections = NULL;
	}
	else if (list[0] && list[0]->type == PARENTHESIS)
	{
		list[0]->sub_list = lexer(list[0]->s);
		parser(list[0]->sub_list, mini);
		list[2] = list[0];
	}
